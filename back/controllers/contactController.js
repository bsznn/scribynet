...
export const addContact = async (req, res) => {
  try {
    ...
    if (!content || !name || !subject || !email) {
      return res.status(400).json({ error: 'Tous les champs sont obligatoires : content, name, subject, email.' });
    }
    ...
    return res.status(201).json({ message: 'Message de contact enregistré avec succès.', data: contact });
  } catch (error) {
